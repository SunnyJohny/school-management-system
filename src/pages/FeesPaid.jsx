...
const FeesPaidReport = () => {
  const { state, searchByKeyword, searchByDate, calculateTotalFeesPaid } = useMyContext();
  ...
  }, [state.payments, fromDate, toDate, searchKeyword, searchByDate, searchByKeyword]);
  ...
  const renderTable = () => (
    <table className="table-auto w-full border-collapse border border-gray-300">
      ...
      <tbody>
        {itemsToDisplay.map((fee, index) => (
          ...
          >
            ...
            <td className="border border-gray-300 px-4 py-2">{fee.studentName}</td>
            <td className="border border-gray-300 px-4 py-2">{fee.studentClass}</td>
            <td className="border border-gray-300 px-4 py-2">{fee.studentId}</td>
            <td className="border border-gray-300 px-4 py-2">{fee.totalAmount}</td>
            <td className="border border-gray-300 px-4 py-2">{fee.timestamp?.toDate?.().toLocaleDateString() || 'Invalid Date'}</td>
            <td className="border border-gray-300 px-4 py-2">{fee.paymentMethod}</td>
      ...
      <tfoot>
        <tr className="bg-gray-200">
          <td className="border border-gray-300 px-4 py-2" colSpan="4"><strong>Total</strong></td>
          <td className="border border-gray-300 px-4 py-2"><strong>₦{totalFeesPaid}</strong></td>
  ...
  const ReceiptModal = ({ saleInfo, onClose }) => {
    const amountInWords = convertToWords(saleInfo.totalAmount);
    ...
    const printReceipt = () => {
      ...
      printWindow.document.write(`
        ...
        <body>
          <div class="header">
            <h2>${saleInfo.schoolName || "School Name"}</h2>
            <p>Receipt Number: ${saleInfo.id}</p>
          ...
          <div class="receipt-details">
            ...
            <p><strong>Total Amount:</strong> ₦${saleInfo.totalAmount.toFixed(2)}</p>
          ...
          .join("")}
              <tr>
                ...
                <td><strong>₦${saleInfo.totalAmount.toFixed(2)}</strong></td>
              </tr>
            </tbody>
          </table>
          <p>Attendant: ${saleInfo.attendant || "Staff Name"}</p>
          <div class="footer">
            <hr />
        ...
        <div className="bg-white rounded-lg p-6 w-11/12 max-w-lg print:shadow-none">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={onClose}>
              <MdClose size={24} className="cursor-pointer text-gray-600 hover:text-gray-800" />
            </button>
          </div>
          <div className="text-sm">
            <p><strong>Receipt No:</strong> {saleInfo.id}</p>
            <p><strong>Student Name:</strong> {saleInfo.studentName}</p>
            <p><strong>Class:</strong> {saleInfo.studentClass}</p>
            <p><strong>Guardian Phone:</strong> {saleInfo.guardianPhone}</p>
            <p><strong>Total Amount:</strong> ₦{saleInfo.totalAmount}</p>
            <p><strong>Amount in Words:</strong> {amountInWords} Only</p>
            <p><strong>Payment Method:</strong> {saleInfo.paymentMethod}</p>
            <p><strong>Term:</strong> {saleInfo.term}</p>
            ...
            <table className="table w-full border-collapse border border-gray-300">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td className="border border-gray-300 px-4 py-2"><strong>₦{saleInfo.totalAmount.toFixed(2)}</strong></td>
                </tr>
              </tbody>
            </table>
          </div>
  ...
  const renderPaginationButtons = () => {
    ...
    return (
      <div className="flex justify-between">
        ...
        <button className="bg-blue-500  ml-2 text-white px-4 py-2 rounded-md" onClick={saveAndPrintTable}>
          Print
        </button>
  ...
  const calculateTodayFees = () => {
    ...
    return state.payments
      .filter((payment) => new Date(payment.timestamp.toDate()).toLocaleDateString() === today) // Filter payments by today's date
      .reduce((total, payment) =>
        total + payment.items.reduce((acc, item) => acc + parseFloat(item.amount), 0), 0); // Sum the amounts from the items array
  };
      ...
      <div className="ml-8 flex-1">
        <div className="mb-8 p-2">
          ...
          <div className="flex flex-wrap p-2 md:space-x-4 space-y-4 md:space-y-0">
            {renderStatCard('Total Revenue', `₦${totalFeesPaid}`, 'blue', faChartLine)}
            {renderStatCard('Total Fees', `₦${totalFeesPaid}`, 'green', faShoppingCart)}
            {renderStatCard('Today Fees', `₦${calculateTodayFees().toFixed(2)}`, 'red', faCalendarAlt)}
        ...
        <div className="mb-4">
          <p><strong>Transactions by Date:</strong></p>
        ...
        <div className="mb-8">
          <div className="table-container overflow-x-auto overflow-y-auto" style={{ maxHeight: '300px' }} ref={tableRef}>
            <div className="mb-4 text-center">
              ...
              <p><strong>Selected Date Period:</strong> {selectedDateOption}</p>
              <p>Report Printed On: {new Date().toLocaleDateString('en-US', { day: '2-digit', month: '2-digit', year: 'numeric' })}</p>
            </div>
            {renderTable()}
